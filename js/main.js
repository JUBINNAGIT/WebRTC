...
if (ROOM !== '') {
  ...
  socket.emit('create', ROOM, 10, USER);
}

socket.on('created', function (room, user){
  console.log('Created room: ' + room);
  isInitiator = true;
});

socket.on('join', function (room, user, userCount){
  console.log('This peer is the initiator of room: ' + room + ' ' + userCount);
  isChannelReady = true;
  joinUser = user;
  maybeStart(user);
});
...
function sendMessage(message){
  ...
  console.log('Room Owner sending message: ', message);
...
function doCall(user) {
  ...
  pc.createOffer(setLocalAndSendMessage.bind(this, user), handleCreateOfferError);
}
